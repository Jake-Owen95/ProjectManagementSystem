...
def reg_user(user_names):
    ...
    user_exists = None
    new_username = input("Register user name: ")
    if new_username in user_names:
        user_exists = True
    while user_exists == True:
        if new_username in user_names:
            print("\nUser name already exists.")
            new_username = input("Enter another Username: ")
        else:
            user_exists = False
    while registered == False:
        new_password = input("Register user password: ")
...
def add_task(user_names):
    ...
    while task_entered == False:
        ...
        if task_user not in user_names:
            ...
            new_task.write(
                "\n{}, {}, {}, {}, {}, {}".format(task_user, task_title, task_descrip, task_date, task_due_date,
                                                  task_complete))
            task_entered = True
            new_task.close()
...
##################### Function for display all tasks###################
...
def view_all():
    ...
    for line in all_tasks:
        ...
        print(" Task Complete? : {}".format(temp[5]))
        num += 1
    all_tasks.close()
...
def view_mine(username, user_names):
    with open("tasks.txt", "r") as file:
        ...
        print("\n Select a task by using its number")
        ...
        if choice == "b":
            ...
            if choice > len(user_tasks) - 1:
                print("\nError: no task with that number!")
            else:
                if "yes" in user_tasks[choice]:
                    ...
                    if choice_2 == "m":
                        ...
                        edit_task(all_tasks, choice, user_names) #<<user_task
